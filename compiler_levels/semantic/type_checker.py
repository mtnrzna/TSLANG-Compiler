...
class TypeChecker(NodeVisitor):
    ...
    def visit_Stmt7(self, node, table):
        ...
        if table:
            ...
            if res != function_type:
                self.semantic_errors.add_error({"message": f"{node.expr.lineno}: Returning wrong type for function '{function_name}'. must return: '{function_type}', you're returning: '{res}'", "lineno":node.expr.lineno})
        # did'nt found the corresponding function to this return
        else:
            self.semantic_errors.add_error({"message": f"{node.expr.lineno}: Shouldn'n have put return here!","lineno": node.expr.lineno})
    ...
    def visit_Expr1(self, node, table):
        ...
        if isinstance(function_symbol, FunctionSymbol):
            ...
            if len(parameters) != len(arguments):
                # handle semantic error
                    self.semantic_errors.add_error({"message": f"{node.clist.lineno}: Function '{function_iden}' expected {len(parameters)} arguments but got {len(arguments)} instead", "lineno": node.clist.lineno})
        ...
        #function is not declared but it can be called becasue of error handling, it returns "Nil"        
            ...
            if not result:
                self.semantic_errors.add_error({"message": f'{node.iden.lineno}: Function: \'{function_iden}\' not defined', "lineno": node.iden.lineno})
                ...
                self.semantic_errors.add_error({"message": f'{node.iden.lineno}: \'{function_iden}\' is not a function', "lineno": node.iden.lineno})
                return result.type
    ...
    def visit_Expr2(self, node, table):
        ...
        if type_of_array_iden == "Array" and type_of_array_index == "Int":
            ...
            self.semantic_errors.add_error({"message": f'{node.expr.lineno}: Id of the aray must of type \'Array\'!', "lineno": node.expr.lineno})
    ...
    def visit_Expr4(self, node, table):
        ...
        if first_operand != second_operand:
            self.semantic_errors.add_error({"message": f'{node.expr.lineno}: Two sides of \'{operator}\' must be of same type! expr1 is of type: \'{first_operand}\' and expr2 is of type: \'{second_operand}\' ', "lineno":node.expr.lineno})
            return "Nil"
    ...
    def visit_Expr5(self, node, table):
        ...
        if operand == "Int" :
            ...
            self.semantic_errors.add_error({"message": f'{node.expr.lineno}: The operand of the \'{operator}\' must be of type \'Int\' or \'Nil\' ', "lineno":node.expr.lineno})
            return "Nil"
        else:
            self.semantic_errors.add_error({"message": f'{node.expr.lineno}: The operand of the \'{operator}\' must be of type \'Int\' ', "lineno":node.expr.lineno})
            return "Nil"
    ...
    def visit_Iden(self, node, table):
        ...
        if not symbol:
            self.semantic_errors.add_error({"message": f"{node.lineno}: '{name}' is not declared", "lineno":node.lineno})
